...
mgrs, latlong (dec & deg/min/sec), utm, CGRS
Author: Tyler Weaver
...
class CoordTranslator:
    ...
    def AsCGRS(self, coord):
        '''! \brief Returns a CGRS string'''
        
        coord = self.AsLatLong(coord, dms=True)
        coord = self.LLtoCGRS(coord)
    ...
    def LLtoCGRS(self, coord):
        ...
        swlatoff = 58	#// 290000N
        swlonoff = 122	#// 610000E
        nelatoff = 76	#// 383000N
        nelonoff = 144	#// 720000E
        ...
        if (((latoff >= 0 ) and (lonoff >= 0))
            and ((lonoff < nelonoff-swlonoff)
                 and (latoff < nelatoff-swlatoff))):
            klat = killlatbase + latoff
            klon1 = ord(killlonbase[0])
            klon2 = ord(killlonbase[1])
            klon2 += lonoff
            while ( klon2 > 90):
                klon2 = klon2 - 26
                klon1 += 1
            
            killbox = "%d%s%s%d" %(klat, chr(klon1), chr(klon2), self._getSubKillBox(coord))
            #getSubKillBox()
        else:
            killbox=""
            killsubbox=""
            print("Coordinate is outside the AOR")

        return killbox
    
    def _getSubKillBox(self, coord):
        kb = ""
        latdeg = coord[0]
        latmin = coord[1]
        latsec = coord[2]
        ...
        if (latmin<10):
            kb = 7
        if ((latmin >=10) and (latmin<20)):
            kb = 4
        if ((latmin >=20) and (latmin<30)):
            kb = 1
        if ((latmin >=30) and (latmin<40)):
            kb = 7
        if ((latmin >=40) and (latmin<50)):
            kb = 4
        if (latmin >=50):
            kb = 1

        if ((lonmin >=10) and (lonmin<20)):
            kb += 1
        if ((lonmin >=20) and (lonmin<30)):
            kb = kb + 2
        if ((lonmin >=40) and (lonmin<50)):
            kb += 1
        if ((lonmin >=50) and (lonmin<60)):
            kb = kb + 2
...
if __name__ == '__main__':
    ...
    print a.AsCGRS([34.213352, 70.291130])
